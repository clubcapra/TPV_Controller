...
/// @brief Servo feedback control
/// @param servo Pointer to the servo object
void fb_servo_update(fb_servo *fb_servo_handle)
{
  // Calculate the servo angle based on the input signal
  fb_servo_calc_angle(fb_servo_handle);

  if (fb_servo_handle->valid == 0)
  {
    // Set the servo position to the neutral position
    fb_servo_handle->servo_handle.write(90);
  }
  else
  {
    // Calculate the new command based on the rate and rate gain
    fb_servo_handle->cmd = fb_servo_handle->cmd + fb_servo_handle->rate_gain * fb_servo_handle->rate;

    // Calculate the error between the command and the current position
    float error = (fb_servo_handle->cmd - fb_servo_handle->pos);

    // Update the integral term of the controller and constrain it to the maximum value
    fb_servo_handle->i = constrain(fb_servo_handle->i + error, -fb_servo_handle->i_max, fb_servo_handle->i_max);

    // Calculate the servo command based on the error and the gains, and constrain it to the valid range
    float servo_command = constrain((error * fb_servo_handle->kp_gain + fb_servo_handle->i * fb_servo_handle->ki_gain) + 90, 0, 180);

    // Write the servo command to the servo motor
    fb_servo_handle->servo_handle.write(servo_command);
  }
}

/// @brief Calculate the angle of the servo
/// @param fb_servo_handle Pointer to the servo object
void fb_servo_calc_angle(fb_servo *fb_servo_handle)
{
  ...
  {
    // Signal has not been read recently, update servo state
  ...
  else
  {
    ...
    {
      // Signal is not valid, update servo state
      fb_servo_handle->valid = 0;
      fb_servo_handle->last_command = micros();
    }
    else
    {
      ...
      // Calculate duty cycle and angle
      ...
      // Check for servo turns and update position
...
/// @brief Interrupt service routine for reading the feedback signal of servo 1
void interupt1()
{
  // If the feedback signal is high
  if (digitalRead(SERVO1_FB_PIN))
  {
    // Record the time of the falling edge of the feedback signal
    servo1.fall = micros();
    servo1.tHigh = servo1.fall - servo1.rise;
  }
  else
  {
    // Record the time of the rising edge of the feedback signal
    servo1.rise = micros();
...
/// @brief Interrupt service routine for reading the feedback signal of servo 2
void interupt2()
{
  // If the feedback signal is high
